...
- **阶段二** — 带角色上下文<span style="color:red;font-weight:bold">并行</span>转换各章节为结构化剧本
...
### 4. ⚡ LLM 并行加速
多章节<span style="color:red;font-weight:bold">并行</span>调用 LLM，耗时从 `N×T`（串行）降至约 `1×T`（并行）。  
实测 10 章从 ~100s 缩至 ~20s，大幅缩短等待时间。🐳 进度页还有小鲸鱼游来游去，直观体现并行处理！
